...
export const createCaseStudy = async (data) => {
  ...
  const [result] = await db.query(
    ...
    (title, slug, category_id, short_description, content, is_featured)
    VALUES (?, ?, ?, ?, ?, ?)`,
    [
      title,
      slug,
...
export const updateCaseStudy = async (id, data) => {
  ...
  await db.query(
    `UPDATE case_studies
     SET title=?, slug=?, category_id=?, short_description=?, content=?, is_featured=?
     WHERE id=?`,
    [
      title,
      slug,
...
export const createCategory = async (name) => {
  await db.query(
    "INSERT INTO case_study_categories (name) VALUES (?)",
    [name]
  );
};
...
export const deleteCategory = async (id) => {
  await db.query(
    "DELETE FROM case_study_categories WHERE id=?",
    [id]
  );
};